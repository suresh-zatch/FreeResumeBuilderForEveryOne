...
import { Rocket, Plus, Trash2, ChevronUp, ChevronDown, ExternalLink, X } from 'lucide-react';
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between border-b border-gray-200 pb-3">
        <div>
          ...
          <p className="text-xs text-gray-500">Showcase software, web apps, research, or portfolio achievements.</p>
        </div>
        <button
          type="button"
      ...
      {projects.length === 0 ? (
        <div className="text-center py-8 border-2 border-dashed border-gray-200 rounded-xl bg-gray-50">
          <p className="text-sm text-gray-500 mb-2">No projects added yet.</p>
          <button
            type="button"
            onClick={handleAddProject}
        ...
        <div className="space-y-4">
          {projects.map((item, index) => (
            ...
            >
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    placeholder="e.g. OmniAI - Code Assistant"
                ...
                <div>
                  <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center gap-1">
                    <ExternalLink className="w-3 h-3 text-gray-400" /> Live Demo URL
                  </label>
                  <input
                    type="text"
                    value={item.link || ''}
                ...
                <div className="md:col-span-2">
                  <label className="block text-xs font-medium text-gray-700 mb-1 flex items-center gap-1">
                    <GithubIcon /> GitHub Repository URL
                  </label>
                  <input
                    type="text"
              ...
              <div>
                ...
                <div className="flex flex-wrap gap-1.5 min-h-[32px] p-2 bg-gray-50 border border-gray-200 rounded-lg mb-2">
                  {(item.technologies || []).length === 0 ? (
                    <span className="text-xs text-gray-400 italic">No tech tags added. Type below and click Add.</span>
                  ) : (
                    item.technologies.map((tech, techIdx) => (
                      <span
                        key={techIdx}
                        className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-md text-xs font-medium bg-blue-50 text-blue-700 border border-blue-100"
                      >
                        {tech}
                        <button
                          type="button"
                ...
                <div className="flex items-center gap-2">
                  <input
                    ...
                    placeholder="Add technology (e.g. Next.js, Python) and press Enter"
              ...
              <div>
                <label className="block text-xs font-medium text-gray-700 mb-1">Project Description & Impact</label>
                <textarea
                  rows={3}
                  value={item.description}
                  onChange={(e) => handleItemChange(index, 'description', e.target.value)}
                  placeholder="Describe the architecture, features, metrics, and key technical accomplishments..."
